...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <div class="container">
    ...
    <form action="${ctx}/room/updateroom.do" method="post" onsubmit="return verify()">
			  ...
			  <div class="span5 ">
			      ...
			      <select name="roomStateID" class="cboone inputone">
		            <c:forEach items="${listTwo}" var="item">
			          <option value="${item.id}" <c:if test="${item.id==listPo.roomStateID}" >selected="selected"</c:if>>
			  ...
			  <div class="span5">
			      ...
			      <select name="guestRoomLevelID" class="cboone inputone">
		            <c:forEach items="${listOne}" var="item">
			          <option value="${item.id}" <c:if test="${item.id==listPo.guestRoomLevelID}">selected="selected"</c:if>>
			            ${item.attributeDetailsName}
			          </option>
			        </c:forEach> 
		          </select>
 ...
 <script type="text/javascript">
    ...
    function deletefunction(){
     parent.document.getElementById('Mainid').src='${ctx}//room/roomSet.do';
   }
