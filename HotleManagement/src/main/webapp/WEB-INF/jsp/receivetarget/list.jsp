...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <div class="container" >
    <div class="span5">
	    <div class="row-fluid">
		    ...
		    <form action="${ctx}/allreceive" method="post" style="float: left;">
       ...
       <table class="table table-condensed table-bordered table-striped" id="tableid">
	      ...
	      <tbody id="tbody">
	        <c:forEach items="${list}" var="item">
		        ...
		        <%--     <c:if test="${item.targetTypeID==52  }">
		        <td>首次</td>
		          </c:if>
		           <c:if test="${item.targetTypeID==53  }">
		           <td>熟客</td>
		          </c:if>
		            <c:if test="${item.targetTypeID==54  }">
		           <td>VIP</td>
		          </c:if> --%>
		           <c:if test="${item.targetTypeID==55  }">
		           <td>散客</td>
		          </c:if>
		           <c:if test="${item.targetTypeID==56  }">
		           <td>团队</td>
		          </c:if>
		          <td>${item.teamName}</td>
		          <td>${item.teamCode}</td>
 ...
 var path="/HotelManagement";
   ...
   var chk_value=[];
  	...
  	if(chk_value!=""){
		if(chk_value.toString().indexOf(",")>0){
		   ...
		   window.location.href="${ctx}/updatereceive?id="+chk_value;
		}
	}else{
	  alert("请选择一条数据进行修改");
	}
  ...
  function addfunction(){
  window.location.href="${ctx}/addss";
  }
   ...
   var chk_value=[];
     ...
     if(flag){
  	  window.location.href="${ctx}/delectreceive?id="+chk_value;
  	}
  	}else{
  ...
  $(".tcdPageCode").createPage({
     ...
     location.href="${ctx}/allreceive?xyy="+p+"&txtname="+txtname;
     }
   });
 </script>
<%--    <div id="div01">
