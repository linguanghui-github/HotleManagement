...
import cn.hotel.entity.PredeterMine;
...
public class PredetermineController {
	...
	public ModelAndView tolist(HttpServletRequest request, Integer currentPage,
		...
		PageInfo<PredeterminePo> vo = new PageInfo<PredeterminePo>();
	...
	@ResponseBody
	@RequestMapping(value="/selectTarget",produces = "text/html;charset=UTF-8")
	...
	public ModelAndView toupdate(Integer id){
		...
		int zhengShu=0;
		String nameString="";
		List<Room> roomSetPolist=new ArrayList<Room>();
		Room roomSetPo=null;
		int type=0;
		...
		if (lvKeId==0) {
			...
			for (int i = 0; i < teamList.size(); i++) {
				...
				roomSetPo=roomSetService.getRoom(teamList.get(i).getRoomID());
			...
			for (int i = 0; i < lvKeList.size(); i++) {
				...
				roomSetPo=roomSetService.getRoom(lvKeList.get(i).getRoomID());
				roomSetPolist.add(roomSetPo);
	...
	public ModelAndView delete(String id){
		...
		for (int i = 0; i < FenGe.length; i++) {
			...
			room.setId(predeterminePo.getRoomID()); 								  //查询房间信息
			room.setRoomStateID(1);													  //给对象赋新的房间状态
				
			this.roomSetService.updateByIdToRoomState(room);						  //更改房间状态				
		}
		mv=new ModelAndView("redirect:/Predetermine/tolist.do");
		return mv;
	}
	...
	public ModelAndView arrangeRoom(String id,Integer tiaoZhuang){
		...
		Room roomSetPoToRoomState=new Room();
		for (int i = 0; i < FenGe.length; i++) {
            this.predetermineService.updatePredetermineStateID(Integer.parseInt(FenGe[i]));              //修改预订状态
            PredeterminePo predeterminePo=this.predetermineService.findById(Integer.parseInt(FenGe[i])); //查询预订信息
            Room roomSetPo=this.roomSetService.getRoom(predeterminePo.getRoomID());              //查询房间信息
